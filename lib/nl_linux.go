...
func executeInNetns(newNs, curNs NsHandle) (func(), error) {
	var (
		err       error
		moveBack  func(NsHandle) error
		closeNs   func() error
		unlockThd func()
	)
	restore := func() {
	...
	if newNs.IsOpen() {
		...
		if !curNs.IsOpen() {
			if curNs, err = Get(); err != nil {
				restore()
				return nil, fmt.Errorf("could not get current namespace while creating netlink socket: %v", err)
			}
			closeNs = curNs.Close
		}
		if err := Set(newNs); err != nil {
			restore()
...
func SubscribeAt(newNs, curNs NsHandle, protocol int, groups ...uint) (*NetlinkSocket, error) {
	c, err := executeInNetns(newNs, curNs)
	if err != nil {
		return nil, err
	}
